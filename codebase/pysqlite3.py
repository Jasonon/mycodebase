...
class RecordCollection(object):
    ...
    def as_list(self,has_header=True):

        rows = list(self)
        header = [r.keys() for r in rows ][0]
        values = [r.values() for r in rows]
        if has_header:
            values.insert(0,tuple(header))
...
class Database(object):
    ...
    def query(self, query, fetchall=False, **params):
        ...
        cursor = self.db.execute(query)  # TODO: PARAMS GO HERE
        result = self.db.fetchall()
        
        key_list = [key[0] for key in cursor.description]
        # Row-by-row Record generator.
        row_gen = (Record(key_list, row) for row in result)
    ...
    def insert(self,sql,data):
        '''INSERT INTO persons (key, fname, lname, email) VALUES(?,?,?,?)'''
        self.write(sql=sql,data=data)
